...
func FormatProfileMarkdown(rules []db.WritingRule, corrections []db.CorrectionRecord) string {
	...
	lines = append(lines, fmt.Sprintf("_Generated at %s — if this is old, exports are failing; check margin logs._", time.Now().UTC().Format(time.RFC3339)))
	lines = append(lines, "_For AI agents: apply rules matching the writing type. General rules always apply._")
	lines = append(lines, "_Resolve conflicts using the latest explicit user correction and source facts, then audience/channel-specific guidance, then general style. Preserve direct quotations and approval boundaries._")
	lines = append(lines, "_Casual texts and DMs may use fragments and omit final periods when supported by relationship evidence. Professional prose uses complete sentences and terminal punctuation. Examples do not override these channel boundaries or establish facts for new writing. Entries under Kill Words may be editorial instructions; interpret their meaning instead of treating an entire instruction as a banned phrase._")
...
func ExportProfile(dbPath string, target string) error {
	...
	if _, statErr := os.Stat(CodexDir()); statErr == nil {
		_ = writeCodexAgentsMD(FormatCodexAgentsMD(rules, corrections))
	}
