...
## Re-baseline on repaired harness — 2026-09-12 (poolside)

The March results above were scored on a ~4%-recall proxy with unfiltered
data (positive voice samples rendered as "avoid" examples, verbatim-sentence
slop matching, register map divergent from production). Re-run after the
fidelity fixes (commit 8b83e3e + a84ad78):

| Architecture | Pass rate | Mean dim | Mech issues | Note |
|---|---|---|---|---|
| null (zero-shot) | 59.3% | 39.9 | 41 | control |
| a — rules dump | 25.9% | 45.3 | 38 | rules hurt pass rate; prose dims improve |
| c — two-pass editor | **66.7%** | 44.0 | **16** | lead holds; edit pass halves mechanical issues |
| d — corrections | 51.9% | 42.9 | 31 | |
| e — hybrid | 44.4% | 42.6 | 39 | corrections+rules underperform d alone |
| skill — production path | 63.0% | **44.7** | 19 | first-ever score of what users actually run |

### Per-type (coached vs null)

- **slack now 3/3 across every coached arch** — was the -4 regression case;
  typeConstraint + register parity appear to have fixed it
- **pitch: 0/3 everywhere including null** — systematic failure, needs
  investigation (prompt or rubric, not coaching)
- **general: 0/3 in all coached archs** vs 0/3 null — hard type

### Read

- The two-pass editor result **replicates directionally** on a trustworthy
  checker — the edit pass is doing real mechanical repair (41→16 issues).
- The production path (skill) is second-best and beats corrections-only —
  the shipped pipeline works; it is not embarrassed by the lab variants.
- Rules-dump is actively harmful to pass rate while lifting prose
